...
import { CATEGORY_CONTENT, NEW_CATEGORY_NAMES_ES } from "../src/lib/content.es";
import { buildNicheContentES } from "../src/lib/niche-content.es";
import type { FaqItem, ContentSection } from "../src/lib/content.es";
import type { NicheType } from "../src/lib/niche-types";

const ROOT = path.join(path.dirname(fileURLToPath(import.meta.url)), "..");
const CACHE_DIR = path.join(ROOT, "scripts", ".cache", "translate-content-sl");
const ES_CACHE_DIR = path.join(ROOT, "scripts", ".cache", "translate-content-es");
...
function validateSl(key: string, esSrc: string, sl: string): string[] {
  const errs: string[] = [];
  if (!sl?.trim()) errs.push(`${key}: empty`);
  if (CYRILLIC.test(sl)) errs.push(`${key}: cyrillic`);
  if (ES_MARKERS.test(sl)) errs.push(`${key}: spanish marker`);
  for (const ph of ["${b}", "${name}", "${topic}", "${key}", "${brand}", "${lc(name)}"]) {
    if (esSrc.includes(ph) && !sl.includes(ph)) errs.push(`${key}: missing placeholder ${ph}`);
  }
  if (esSrc.includes(BRAND) && !sl.includes("${b}") && !sl.includes("${brand}"))
    errs.push(`${key}: missing brand placeholder`);
  if (esSrc.includes(NAME) && !sl.includes("${name}")) errs.push(`${key}: missing name placeholder`);
  return errs;
}
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: {
          ...
          "X-Title": "recenze-ceny-sl-translate",
        },
        body: JSON.stringify({
          model,
          messages: [
      ...
      const parsed = parseJsonLoose(content) as {
        items?: Array<{ key: string; sl?: string; es?: string }>;
      };
      if (!parsed.items?.length) throw new Error(`missing items array: ${content.slice(0, 200)}`);
      const out: Record<string, string> = {};
      for (const row of parsed.items) {
        const sl = row.sl ?? row.es;
        if (!sl) throw new Error(`missing sl for key ${row.key}`);
        out[row.key] = sl;
      }
      return out;
    } catch (e) {
...
async function translateItems(
  ...
  const system = `Si copywriter SEO za trgovino na slovenskem trgu (plačilo ob prevzemu, dostava v Sloveniji).
...
async function translateWithRetryChunk(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    errors = [];
    for (const item of items) {
      errors.push(...validateSl(item.key, item.es, mapped[item.key]));
    }
...
function extractNewCategoryItems(): TextItem[] {
  const items: TextItem[] = [];
  for (const [slug, { name, short }] of Object.entries(NEW_CATEGORY_NAMES_ES)) {
...
function extractNicheItems(niche: NicheType): TextItem[] {
  ...
  const built = buildNicheContentES(slug, NAME, SHORT);
...
function writeContentSl(
  ...
  const lines: string[] = [
    ...
    "// Generated by scripts/translate-content-sl.mjs (LLM from IT).",
    "",
  ...
  lines.push(
    ...
    "};",
    "",
    "export const NEW_CATEGORY_NAMES_SL: Record<string, { name: string; short: string }> = {",
    Object.entries(NEW_CATEGORY_NAMES_ES)
      .map(
        ([slug]) =>
    ...
    "      const rich = buildNicheContentSL(slug, exact.nameHi, exact.shortDescHi);",
    "      return {",
    "        ...exact,",
    ...
    "    return exact;",
    "  }",
    "  const named = NEW_CATEGORY_NAMES_SL[slug];",
    "  if (named) return buildNicheContentSL(slug, named.name, named.short);",
    "  return {",
    "    slug,",
    ...
    "    keywordsHi: [\"prehransko dopolnilo zdravje\", \"naravne kapsule Slovenija\"],",
    "  };",
    "}",
    "",
  );

  fs.writeFileSync(path.join(ROOT, "src/lib/content.sl.ts"), lines.join("\n"), "utf8");
}
...
function writeNicheSl(nicheData: Record<NicheType, Record<string, string>>): void {
  ...
  const lines: string[] = [
    ...
    "// block — Slovenian mirror of niche-content.es.ts (Slovenia market).",
    "// Generated by scripts/translate-content-sl.mjs (LLM from IT).",
    "",
    "import type { CategoryContent, ContentSection, FaqItem } from \"./content.sl\";",
    "import type { CategoryDescriptor } from \"./category-descriptors.sl\";",
    "import { getCategoryDescriptor } from \"./category-descriptors.sl\";",
    "import { getNicheType, type NicheType } from \"./niche-types\";",
    "",
    writeSectionConst("DELIVERY_SL", d("supplement", "delivery.heading"), d("supplement", "delivery.body")),
    "",
    writeSectionConst("QUALITY_SL", d("supplement", "quality.heading"), d("supplement", "quality.body")),
    "",
  ...
  lines.push(
    ...
    "}",
    "",
    "export function buildNicheContentSL(slug: string, name: string, shortDesc: string): CategoryContent {",
    ...
    "    productSections: () => [safety, DELIVERY_SL, QUALITY_SL],",
  ...
  fs.writeFileSync(path.join(ROOT, "src/lib/niche-content.sl.ts"), lines.join("\n"), "utf8");
}
...
export async function main() {
  ...
  console.log("translate-content-sl — batches:", only?.join(",") ?? "all");
  ...
  console.log("Written src/lib/content.sl.ts and src/lib/niche-content.sl.ts");
}
