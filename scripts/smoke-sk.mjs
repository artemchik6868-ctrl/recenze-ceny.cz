/**
 * Slovak locale smoke — skrecenzie.sk
 * Usage: node scripts/smoke-sk.mjs [--base=https://skrecenzie.sk]
 */
const base =
...
const CZ_MARKERS = ["Platba na dobírku", "Kategorie", "Nápověda", "Recenze Ceny", "Doručení"];
...
function checkLocaleLeaks(url, html) {
  for (const marker of [...RO_MARKERS, ...DE_MARKERS, ...HU_MARKERS, ...CZ_MARKERS]) {
...
async function checkUrl(url, { expectHtml = false, attempts = 3 } = {}) {
  for (let tryNo = 1; tryNo <= attempts; tryNo++) {
    try {
      ...
      if (!html.includes('lang="sk"') && !html.includes("sk-SK")) {
        console.log(`WARN ${url} — missing lang=sk`);
      }
      return html;
    } catch (e) {
      if (tryNo < attempts) {
