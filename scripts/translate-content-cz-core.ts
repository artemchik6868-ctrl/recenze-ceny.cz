/**
 * LLM translate content.hu.ts + niche-content.hu.ts → native *.cs.ts (Czech Republic)
 */
import fs from "fs";
...
const SYSTEM = `Translate Hungarian TypeScript content modules to native Czech (cs-CZ) for recenze-ceny.cz.
Output ONLY valid TypeScript. Brand: Recenze Ceny. Market: Česká republika, platba na dobírku.
Rename HU exports to CS equivalents. Fix .hu imports to .cs. Currency: Kč.`;
...
async function main() {
  ...
  const jobs = [
    { hu: "src/lib/content.hu.ts", cs: "src/lib/content.cs.ts" },
    { hu: "src/lib/niche-content.hu.ts", cs: "src/lib/niche-content.cs.ts" },
  ];
  ...
  for (const job of jobs) {
    ...
    const cache = path.join(CACHE_DIR, path.basename(job.cs));
    let out: string;
    if (!force && fs.existsSync(cache)) {
      out = fs.readFileSync(cache, "utf8");
    } else {
      const src = fs.readFileSync(path.join(ROOT, job.hu), "utf8");
      out = await callLLMText(SYSTEM, `Translate to Czech:\n\n${src}`, apiKey, url, model);
