/**
 * Audit SK locale templates for CZ/DE/HU/Cyrillic leaks + hardcoded wrong lang in UI.
 * Run: node scripts/audit-locale-sk.mjs
 */
import fs from "fs";
...
const { hasNonSlovakLocaleLeak } = load("../src/lib/locale-leak-sk.ts");

const TEMPLATE_GLOBS = [
  "src/lib/i18n.sk.ts",
  "src/lib/content.sk.ts",
  "src/lib/niche-content.sk.ts",
  "src/lib/legal.sk.ts",
  "src/lib/category-descriptors.sk.ts",
  "src/lib/category-content.ts",
  "src/lib/category-page-meta.ts",
  "src/lib/lead-errors.sk.ts",
  "src/lib/seo-alt.ts",
  "src/lib/seo-meta.ts",
  "src/lib/pdp-variants.ts",
  "src/lib/delivery-eta.ts",
  "src/lib/product-facts.sk-labels.ts",
  "src/lib/product-page.tsx",
  "src/components/StickyMobileCta.tsx",
  "src/routes/__root.tsx",
  "src/routes/category.$slug.tsx",
  "src/routes/sprievodca.$slug.tsx",
  "src/data/review-templates-cat.sk.ts",
  "src/data/review-templates-niche.sk.ts",
  "src/data/review-templates-slug.sk.ts",
  "src/data/review-templates-theme.sk.ts",
  "src/lib/reviews.ts",
];
...
function scanStringLiterals(filePath) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const m of line.matchAll(STRING_RE)) {
      ...
      if (/^[\w./-]+$/.test(s) && !/[äöüßÄÖÜáäčďéíĺľňóôŕšťúýžÁÄČĎÉÍĹĽŇÓÔŔŠŤÚÝŽ]/.test(s)) continue;
      if (hasNonSlovakLocaleLeak(s)) {
...
if (leaks) {
  console.log(`\naudit-locale-sk: ${leaks} leak(s) — run translate:ui:sk / fix hardcoded "de"`);
  process.exit(1);
}
console.log("audit-locale-sk: OK");
