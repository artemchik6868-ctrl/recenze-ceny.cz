 ...
 *
 * Usage:
 *   npx tsx scripts/regen-locale-leaks-cz.ts --dry-run
 *   npx tsx scripts/regen-locale-leaks-cz.ts --limit=20
 *   npx tsx scripts/regen-locale-leaks-cz.ts --only=shakes:12541
 *   npx tsx scripts/regen-locale-leaks-cz.ts --bad-titles --dry-run
 *   npx tsx scripts/regen-locale-leaks-cz.ts --garbled-titles --dry-run
 *   npx tsx scripts/regen-locale-leaks-cz.ts --bad-body --dry-run
 *   npx tsx scripts/regen-locale-leaks-cz.ts --bad-cards --dry-run
 */
...
async function loadBadCardCandidates(): Promise<ContentRow[]> {
  ...
  for (const pattern of patterns) {
    try {
      ...
      console.warn(`regen-locale-leaks-cz: bad-cards ilike ${pattern} failed:`, err);
    }
  }
  return [...byKey.values()];
}
...
async function loadBadTitleCandidates(): Promise<ContentRow[]> {
  ...
  for (const pattern of patterns) {
    try {
      ...
      console.warn(`regen-locale-leaks-cz: bad-title ilike ${pattern} failed:`, err);
    }
  }
  return [...byKey.values()];
}
...
async function loadGermanDisplayCandidates(): Promise<ContentRow[]> {
  ...
  for (const pattern of patterns) {
    try {
      ...
      console.warn(`regen-locale-leaks-cz: ilike ${pattern} failed:`, err);
    }
  }
  return [...byKey.values()];
}
...
async function loadGarbledTitleCandidates(): Promise<ContentRow[]> {
  ...
  for (const source of PIPELINE_SOURCES) {
    try {
      ...
      console.warn(`regen-locale-leaks-cz: garbled skip source ${source}:`, err);
    }
  }
  return all;
}
...
async function loadBadBodyCandidates(): Promise<ContentRow[]> {
  ...
  for (const pattern of patterns) {
    try {
      ...
      console.warn(`regen-locale-leaks-cz: bad-body ilike ${pattern} failed:`, err);
    }
  }
  return [...byKey.values()];
}
...
async function loadContentRows(): Promise<ContentRow[]> {
  ...
  for (const source of PIPELINE_SOURCES) {
    try {
      ...
      console.warn(`regen-locale-leaks-cz: skip source ${source} load:`, err);
    }
  }
  return all;
}
...
try {
  ...
  console.error("regen-locale-leaks-cz: DB error", err);
  process.exit(1);
}
if (contentRows.length === 0) {
  console.error("regen-locale-leaks-cz: no product_content rows loaded");
  process.exit(1);
}
...
console.log(
  `\n=== regen-locale-leaks-cz — ${targetKeys.length}/${leakingKeys.length} DB leaking rows, ${catalogOnlyLeaks.length} catalog-only (dry=${dryRun}) pipeline=${PIPELINE_VERSION} ===\n`,
);
