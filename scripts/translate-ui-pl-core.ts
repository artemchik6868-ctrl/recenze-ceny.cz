...
const CACHE_DIR = path.join(ROOT, "scripts", ".cache", "translate-ui-pl");
...
const TS_SYSTEM = `You translate Spanish TypeScript locale files to native Polish (cs-CZ) for a Poland e-commerce site.
...
- Market: Polska, płatność przy odbiorze, dostawa 2–5 dni (NOT mensajería/contrassegno/España/Slovenija)
- Phone example: +48 (NOT +34, NOT +386)
- Cities list: Praha, Kraków, Łódź, Wrocław, Poznań, Gdańsk, Szczecin, Bydgoszcz, etc.
- Rename ES_META → PL_META, getCategoryDescriptorES → getCategoryDescriptorPL where applicable
...
async function translateTsFile(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    const user = `${headerComment}\n\nTranslate this TypeScript chunk to Polish. This is chunk ${i + 1} of ${chunks.length}.\n\n${chunks[i]}`;
  ...
  merged = merged
    ...
    .replace(/ES_META/g, "PL_META")
    .replace(/getCategoryDescriptorES/g, "getCategoryDescriptorPL")
    .replace(/getCategoryDescriptorSL/g, "getCategoryDescriptorPL");
...
export async function main() {
  ...
  const jobs = [
    {
      ...
      sl: "src/lib/i18n.pl.ts",
      header: "// Polish UI strings. Same shape as i18n.it.ts.",
    },
    {
      es: "src/lib/legal.es.ts",
      sl: "src/lib/legal.pl.ts",
      header: "// Polish legal/info pages.",
    },
    {
      es: "src/lib/category-descriptors.es.ts",
      sl: "src/lib/category-descriptors.pl.ts",
      header: "// Polish category descriptors for AI prompts and display titles.",
    },
    {
  ...
  console.log("translate-ui-pl —", jobs.length, "files");
  ...
  console.log("Done — i18n.pl.ts, legal.pl.ts, category-descriptors.pl.ts");
}
