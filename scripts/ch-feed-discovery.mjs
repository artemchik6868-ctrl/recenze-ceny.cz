/**
 * Dry-run CPA feed counts for CH geo (MARKET_GEO=CH).
...
async function countKma() {
  ...
  const ch = all.filter((o) => Array.isArray(o.codes) && o.codes.some(matchesGeo));
  return { network: "KMA", status: "ok", total: all.length, ch: ch.length };
}
...
async function countShakes() {
  ...
  const ch = all.filter((o) => (o.goals ?? []).some((g) => matchesGeo(g.geo)));
  return { network: "Shakes", status: "ok", total: all.length, ch: ch.length };
}
...
async function countAdcombo() {
  ...
  return { network: "Adcombo", status: "ok", total: all.length, ch: ch.length };
}
...
async function countCpagetti() {
  ...
  const ch = page.filter((o) => {
    const geos = o?.in_geo ?? o?.geo ?? [];
    const list = Array.isArray(geos) ? geos : [geos];
    return list.some((g) => matchesGeo(g?.country_code ?? g));
  });
  return { network: "Cpagetti", status: "ok", total: page.length, ch: ch.length, totalReported: data?.info?.total };
}

async function main() {
  console.log(`CH feed discovery (geo=${GEO_CODES.join("/")})\n`);
  const results = [];
  ...
  const networksWithCh = results.filter((r) => r.ch > 0).length;
  if (networksWithCh < 1) {
    console.warn(`\nWarning: only ${networksWithCh} network(s) returned CH offers. Check API keys and CH stream IDs.`);
    if (networksWithCh === 0) process.exitCode = 1;
  } else {
    console.log(`\nOK — ${networksWithCh} networks returned CH offers.`);
  }
}
