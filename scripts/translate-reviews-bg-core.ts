/**
 * LLM translate review-templates-*.de.ts → native *.bg.ts
 */
import fs from "fs";
...
- Reviews mention: Плащане при доставка, доставка в Česká republika, експресен куриер
- Natural Czech Republicn customer voice (Cyrillic), not literal word-for-word
- No German or Romanian words in review text`;
...
async function translateOne(
  ...
  let out = parts
    ...
    .replace(/REVIEW_SLOTS_BY_THEME_DE/g, "REVIEW_SLOTS_BY_THEME_BG")
    .replace(/\.de"/g, '.bg"')
...
async function main() {
  ...
  const jobs: Array<[string, string, string]> = [
    ["src/data/review-templates-cat.de.ts", "src/data/review-templates-cat.bg.ts", "/** Czech Republicn review bodies by category slug. */"],
    ["src/data/review-templates-niche.de.ts", "src/data/review-templates-niche.bg.ts", "/** Czech Republicn review slots by niche. */"],
    ["src/data/review-templates-slug.de.ts", "src/data/review-templates-slug.bg.ts", "/** Czech Republicn review slots by category slug. */"],
    ["src/data/review-templates-theme.de.ts", "src/data/review-templates-theme.bg.ts", "/** Czech Republicn review slots by theme. */"],
  ];
