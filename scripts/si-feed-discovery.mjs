/**
 * Dry-run CPA feed counts for SI geo (MARKET_GEO=SI).
 * Requires .env with CPA keys. Does not write to Supabase.
 *
 * Usage: node scripts/si-feed-discovery.mjs
 */
...
async function countKma() {
  ...
  const si = all.filter((o) => Array.isArray(o.codes) && o.codes.some(matchesGeo));
  return { network: "KMA", status: "ok", total: all.length, si: si.length };
}
...
async function countShakes() {
  ...
  const data = await res.json();
  const all = Array.isArray(data) ? data : data?.offers ?? [];
  const si = all.filter((o) => (o.goals ?? []).some((g) => matchesGeo(g.geo)));
  return { network: "Shakes", status: "ok", total: all.length, si: si.length };
}

async function main() {
  console.log(`SI feed discovery (geo=${GEO_CODES.join("/")})\n`);
  const results = [];
  ...
  const anySi = results.some((r) => r.si > 0);
  if (!anySi) {
    console.warn("\nWarning: no SI offers in sampled feeds. Check API keys and SI stream IDs.");
    process.exitCode = 1;
  } else {
    console.log("\nOK — at least one network returned SI offers.");
  }
}
