/**
 * Persist + regen offers mis-shelved as hearing (KMA «Слух» bucket trap).
 * Usage: npx tsx scripts/regen-hearing-traps.ts [--dry-run]
 */
...
for (const o of offers) {
  ...
  if (expected === "sluch" || expected === "other") continue;
  const stalePersisted = persisted === "sluch";
  const staleSync = syncSlug === "sluch" && expected !== syncSlug;
  const kmaTrap = /слух/i.test(String(categoryField)) && expected !== "sluch";
  if (!stalePersisted && !staleSync && !kmaTrap) continue;
  targets.push({
    key,
    expected,
    reason: `${persisted ?? syncSlug} → ${expected} «${rawTitle.slice(0, 40)}»`,
  });
}

console.log(`\n=== regen-hearing-traps — ${targets.length} offers (dry=${dryRun}) ===\n`);
