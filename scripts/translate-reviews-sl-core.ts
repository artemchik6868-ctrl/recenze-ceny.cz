/**
 * Core: LLM batch translate review *.sl.ts → native *.sl.ts
 */
import fs from "fs";
...
import { REVIEW_SLOTS_BY_THEME } from "../src/data/review-templates-theme.es";
...
const CACHE_DIR = path.join(ROOT, "scripts", ".cache", "translate-reviews-sl");
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      ...
      const parsed = parseJsonLoose(content) as {
        reviews?: Array<{ i: number; g: "m" | "f"; sl?: string; es?: string }>;
      };
...
async function translateBatch(
  ...
  const system = `Si avtor pristnih mnenj strank za trgovino v Sloveniji (plačilo ob prevzemu, dostava).
Prevedi vsako špansko mnenje v naravno slovenščino (sl-SI).
PRAVILA: samo slovenščina; brez cirilice; brez španskih besed; ohrani ton (osebna izkušnja, roki, dostava).
Uporabi «Slovenija», «plačilo ob prevzemu», «dostava» kjer original omenja plačilo/dostavo.
Podobna dolžina (40–220 znakov).`;
  ...
  const userParts = [
    ...
    priorErrors?.length
      ...
      : "",
    "",
    "Devuelve JSON: { \"reviews\": [{ \"i\": number, \"g\": \"m\"|\"f\", \"sl\": string }, ...] }",
    "Mismo orden, mismo g, mismo i. Solo slovenščina nativa.",
  ];
...
function writeCatSl(bodies: Record<string, Array<{ g: "m" | "f"; text: string }>>) {
  const lines = [
    "/**",
    " * Slovenian review bodies keyed by category slug (parallel to CATEGORY_TEMPLATES slot order).",
    " * Generated by scripts/translate-reviews-sl.mjs (LLM from ES).",
    " */",
    "",
    "export type SlReviewBody = { g: \"m\" | \"f\"; text: string };",
    "",
    "export function sl(g: \"m\" | \"f\", text: string): SlReviewBody {",
    "  return { g, text };",
    "}",
    "",
    "export const CATEGORY_SL_BODIES: Record<string, SlReviewBody[]> = {",
  ];
  ...
  lines.push("};", "");
  fs.writeFileSync(path.join(ROOT, "src/data/review-templates-cat.sl.ts"), lines.join("\n"), "utf8");
}

function writeThemeSl(themes: Record<string, SlotRow[]>) {
  const lines = [
    "/** Slovenian review slots by health/theme. Generated by translate-reviews-sl.mjs */",
    "",
    "import type { ReviewSlot } from \"./review-templates\";",
    "import type { ReviewTheme } from \"@/lib/review-themes.sl\";",
    "",
    "function slot(",
    "  rating: 4 | 5,",
    "  daysAgo: number,",
  ...
  lines.push("};", "");
  fs.writeFileSync(path.join(ROOT, "src/data/review-templates-theme.sl.ts"), lines.join("\n"), "utf8");
}

function writeNicheSl(niches: Record<string, SlotRow[]>) {
  const lines = [
    "/** Slovenian review slot texts by niche. Generated by translate-reviews-sl.mjs */",
    "",
    "import type { ReviewSlot } from \"./review-templates\";",
    "",
    "function slot(",
    ...
    "}",
    "",
    "export const NICHE_TEMPLATES_SL: Record<string, ReviewSlot[]> = {",
  ];
  ...
  lines.push("};", "");
  fs.writeFileSync(path.join(ROOT, "src/data/review-templates-niche.sl.ts"), lines.join("\n"), "utf8");
}
...
function writeSlugSl(inline: Record<string, SlotRow[]>) {
  const lines = [
    "/** Slovenian review slots keyed by category slug. Generated by translate-reviews-sl.mjs */",
    "",
    "import type { ReviewSlot } from \"./review-templates\";",
    "import { REVIEW_SLOTS_BY_THEME } from \"./review-templates-theme.sl\";",
    "",
    "function slot(",
    "  rating: 4 | 5,",
    "  daysAgo: number,",
  ...
  lines.push("};", "");
  fs.writeFileSync(path.join(ROOT, "src/data/review-templates-slug.sl.ts"), lines.join("\n"), "utf8");
}
...
async function main() {
  ...
  if (dryRun) {
    const catCount = Object.keys(CATEGORY_ES_BODIES).length;
    const themeCount = Object.keys(REVIEW_SLOTS_BY_THEME).length;
    const nicheCount = Object.keys(NICHE_TEMPLATES_ES).length;
  ...
  // Categories
  for (const [slug, bodies] of Object.entries(CATEGORY_ES_BODIES)) {
  ...
  // Niches
  for (const [niche, slots] of Object.entries(NICHE_TEMPLATES_ES)) {
  ...
  const { REVIEW_SLOTS_BY_SLUG } = await import("../src/data/review-templates-slug.es");
  for (const slug of SLUG_INLINE) {
    if (!shouldRun(slug, only)) continue;
  ...
  if (dryRun) {
    console.log("dry-run — cache updated, *.sl.ts not written");
    return;
  }

  if (only?.length) {
    console.log("--only: cache updated; run without --only to regenerate all *.sl.ts files");
    return;
  }
  ...
  console.log("Done — review *.sl.ts files written.");
}
