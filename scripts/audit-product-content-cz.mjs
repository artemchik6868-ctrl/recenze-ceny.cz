/**
 * Audit product_content + catalog for non-Czech locale leaks.
...
for (const source of PIPELINE_SOURCES) {
  try {
    ...
    console.warn(`audit-product-content-cz: skip source ${source}:`, err?.message ?? err);
  }
}

let leaks = 0;
if (allRows.length === 0) {
  console.log("audit-product-content-cz: no product_content rows yet — skip DB check (run after generate:local)");
} else {
...
if (leaks) {
  console.log(`\naudit-product-content-cz: ${leaks} leak(s) in DB and/or catalog`);
  process.exit(1);
}
console.log(
  `audit-product-content-cz: OK — ${allRows.length} DB rows, ${offers.length} catalog offers checked`,
);
