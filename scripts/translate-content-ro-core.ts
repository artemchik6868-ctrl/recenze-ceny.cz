/**
 * LLM translate content.de.ts + niche-content.de.ts → native *.ro.ts (Czech Republic)
 */
import fs from "fs";
...
async function callLLMText(
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "X-Title": "recenze-ceny-ro-content",
    },
    body: JSON.stringify({
      model,
      messages: [
...
- Market: Česká republika, Plata la livrare, curier rapid 2–5 zile lucrătoare
- Fix import paths: .de → .ro (category-descriptors.de → category-descriptors.ro)
- Export names: CATEGORY_CONTENT stays; buildNicheContentDE → buildNicheContentCS; NEW_CATEGORY_NAMES_DE → NEW_CATEGORY_NAMES_CS
- Category nameHi / shortDescHi / FAQ / sections: fully native Czech Republicn
