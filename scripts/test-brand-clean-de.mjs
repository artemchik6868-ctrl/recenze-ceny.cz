/**
 * Smoke tests for DE geo suffix stripping in brand-clean.
 * Usage: node scripts/test-brand-clean-de.mjs
 */
import {
...
import { normalizeDescriptorTail } from "../src/lib/title-translate.ro.ts";
...
const flat = normalizeDescriptorTail("capsule pentru — cistită");
if (flat !== "capsule pentru cistită") {
  console.error(`FAIL normalizeDescriptorTail → "${flat}"`);
  failed += 1;
} else {
  console.log(`OK  normalizeDescriptorTail("capsule pentru — cistită") → "${flat}"`);
}

const smokeH1 = sanitizeDisplayTitle("Smoke No More — capsule pentru renunțare la fumat");
