/**
 * CZ ops health check — pipeline, content audit, smoke, secrets, cron.
...
writeFileSync(resolve(cacheDir, "ops-health-cz.json"), JSON.stringify({ at: new Date().toISOString(), failed, steps }, null, 2));

if (failed) {
  console.log(`\nops-health-cz: ${failed} step(s) failed`);
  process.exit(1);
}
console.log("\nops-health-cz: OK");
