/**
 * Smoke audit: DE AI prompts must not leak PL/UA/IT locale markers.
 * Usage: npx tsx scripts/test-prompts-de.mjs
 */
import {
  SYSTEM_PROMPT_DE,
  TRANSLATE_SYSTEM_DE,
...
function check(label, text, opts = { requireDeutschland: true, requireDeutsch: true }) {
  ...
  if (opts.requireDeutschland && !/Deutschland/i.test(text)) {
    console.error(`FAIL ${label}: missing Deutschland`);
    failed += 1;
    return;
  }
...
check("TRANSLATE_SYSTEM_DE", TRANSLATE_SYSTEM_DE, { requireDeutschland: false });
check("FAQ_SYSTEM_DE", FAQ_SYSTEM_DE);
check("QA_HTML_TRUNCATION_HINTS", QA_HTML_TRUNCATION_HINTS_DE.join("\n"), { requireDeutschland: false, requireDeutsch: false });
...
if (deliveryH2 !== "Lieferung und Zahlung in Deutschland") {
