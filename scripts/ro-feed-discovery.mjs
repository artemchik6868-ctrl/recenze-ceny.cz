/**
 * Dry-run CPA feed counts for RO geo (MARKET_GEO=RO).
...
async function countKma() {
  ...
  const ro = all.filter((o) => Array.isArray(o.codes) && o.codes.some(matchesGeo));
  return { network: "KMA", status: "ok", total: all.length, ro: ro.length };
}
...
async function countShakes() {
  ...
  const ro = all.filter((o) => (o.goals ?? []).some((g) => matchesGeo(g.geo)));
  return { network: "Shakes", status: "ok", total: all.length, ro: ro.length };
}
...
async function countAdcombo() {
  ...
  return { network: "Adcombo", status: "ok", total: all.length, ro: ro.length };
}
...
async function countCpagetti() {
  ...
  const ro = page.filter((o) => {
    const geos = o?.in_geo ?? o?.geo ?? [];
    const list = Array.isArray(geos) ? geos : [geos];
    return list.some((g) => matchesGeo(g?.country_code ?? g));
  });
  return { network: "Cpagetti", status: "ok", total: page.length, ro: ro.length, totalReported: data?.info?.total };
}

async function main() {
  console.log(`RO feed discovery (geo=${GEO_CODES.join("/")})\n`);
  const results = [];
  ...
  const networksWithRo = results.filter((r) => r.ro > 0).length;
  if (networksWithRo < 2) {
    console.warn(`\nWarning: only ${networksWithRo} network(s) returned RO offers. Check API keys and RO stream IDs.`);
    if (networksWithRo === 0) process.exitCode = 1;
  } else {
    console.log(`\nOK — ${networksWithRo} networks returned RO offers.`);
  }
}
