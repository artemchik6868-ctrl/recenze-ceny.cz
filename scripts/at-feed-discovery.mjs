/**
 * Dry-run CPA feed counts for AT geo (MARKET_GEO=AT).
 * Usage: node scripts/at-feed-discovery.mjs
 */
...
async function countKma() {
  ...
  const at = all.filter((o) => Array.isArray(o.codes) && o.codes.some(matchesGeo));
  return { network: "KMA", status: "ok", total: all.length, at: at.length };
}
...
async function countShakes() {
  ...
  const at = all.filter((o) => (o.goals ?? []).some((g) => matchesGeo(g.geo)));
  return { network: "Shakes", status: "ok", total: all.length, at: at.length };
}
...
async function countAdcombo() {
  ...
  return { network: "Adcombo", status: "ok", total: all.length, at: at.length };
}
...
async function countCpagetti() {
  ...
  const at = page.filter((o) => {
    const geos = o?.in_geo ?? o?.geo ?? [];
    const list = Array.isArray(geos) ? geos : [geos];
    return list.some((g) => matchesGeo(g?.country_code ?? g));
  });
  return { network: "Cpagetti", status: "ok", total: page.length, at: at.length, totalReported: data?.info?.total };
}

async function main() {
  console.log(`AT feed discovery (geo=${GEO_CODES.join("/")})\n`);
  const results = [];
  ...
  const networksWithAt = results.filter((r) => r.at > 0).length;
  if (networksWithAt < 2) {
    console.warn(`\nWarning: only ${networksWithAt} network(s) returned AT offers. Check API keys and AT stream IDs.`);
    if (networksWithAt === 0) process.exitCode = 1;
  } else {
    console.log(`\nOK — ${networksWithAt} networks returned AT offers.`);
  }
}
