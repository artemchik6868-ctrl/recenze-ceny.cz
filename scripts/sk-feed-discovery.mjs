/**
 * Dry-run CPA feed counts for SK geo (MARKET_GEO=SK).
 * Usage: node scripts/sk-feed-discovery.mjs
 */
...
async function countKma() {
  ...
  const sk = all.filter((o) => Array.isArray(o.codes) && o.codes.some(matchesGeo));
  return { network: "KMA", status: "ok", total: all.length, sk: sk.length };
}
...
async function countShakes() {
  ...
  const res = await fetch(url, {
    headers: { Accept: "application/json", "User-Agent": "skrecenzie-discovery/1.0" },
  });
  if (!res.ok) return { network: "Shakes", status: "error", http: res.status };
  const data = await res.json();
  const all = Array.isArray(data) ? data : data?.offers ?? [];
  const sk = all.filter((o) => (o.goals ?? []).some((g) => matchesGeo(g.geo)));
  return { network: "Shakes", status: "ok", total: all.length, sk: sk.length };
}
...
async function countAdcombo() {
  ...
  return { network: "Adcombo", status: "ok", total: all.length, sk: sk.length };
}
...
async function countCpagetti() {
  ...
  const sk = page.filter((o) => {
    const geos = o?.in_geo ?? o?.geo ?? [];
    const list = Array.isArray(geos) ? geos : [geos];
    return list.some((g) => matchesGeo(g?.country_code ?? g));
  });
  return { network: "Cpagetti", status: "ok", total: page.length, sk: sk.length, totalReported: data?.info?.total };
}

async function main() {
  console.log(`SK feed discovery (geo=${GEO_CODES.join("/")})\n`);
  const results = [];
  ...
  const networksWithSk = results.filter((r) => r.sk > 0).length;
  if (networksWithSk < 2) {
    console.warn(`\nWarning: only ${networksWithSk} network(s) returned SK offers. Check API keys and SK stream IDs.`);
    if (networksWithSk === 0) process.exitCode = 1;
  } else {
    console.log(`\nOK — ${networksWithSk} networks returned SK offers.`);
  }
}
