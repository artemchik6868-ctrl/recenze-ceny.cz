/**
 * Dry-run CPA feed counts for CZ geo (MARKET_GEO=CZ).
...
async function countKma() {
  ...
  const cz = all.filter((o) => Array.isArray(o.codes) && o.codes.some(matchesGeo));
  return { network: "KMA", status: "ok", total: all.length, cz: cz.length };
}
...
async function countShakes() {
  ...
  const cz = all.filter((o) => (o.goals ?? []).some((g) => matchesGeo(g.geo)));
  return { network: "Shakes", status: "ok", total: all.length, cz: cz.length };
}
...
async function countAdcombo() {
  ...
  return { network: "Adcombo", status: "ok", total: all.length, cz: cz.length };
}
...
async function countCpagetti() {
  ...
  const cz = page.filter((o) => {
    const geos = o?.in_geo ?? o?.geo ?? [];
    const list = Array.isArray(geos) ? geos : [geos];
    return list.some((g) => matchesGeo(g?.country_code ?? g));
  });
  return { network: "Cpagetti", status: "ok", total: page.length, cz: cz.length, totalReported: data?.info?.total };
}

async function main() {
  console.log(`CZ feed discovery (geo=${GEO_CODES.join("/")})\n`);
  const results = [];
  ...
  const networksWithCz = results.filter((r) => r.cz > 0).length;
  if (networksWithCz < 2) {
    console.warn(`\nWarning: only ${networksWithCz} network(s) returned CZ offers. Check API keys and CZ stream IDs.`);
    if (networksWithCz === 0) process.exitCode = 1;
  } else {
    console.log(`\nOK — ${networksWithCz} networks returned CZ offers.`);
  }
}
